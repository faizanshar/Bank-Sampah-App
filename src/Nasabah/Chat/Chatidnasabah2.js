...
class Chatidnasabah2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  getPesan() {
    fetch(
      `https://nsj-trash.herokuapp.com/api/pesan/${this.props.route.params.item.penjemput_id}`,
      {
        method: 'GET',
        headers: {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Image
            source={{uri: this.props.route.params.item.foto_penjemput}}
            style={styles.img}
          />
          <Text style={styles.txt}>{this.props.route.params.item.alamat_penjemput}</Text>
        </View>
        <ScrollView
          style={{width: '100%'}}
          ref={(ref) => {
...
export default connect(mapStateToProps)(Chatidnasabah2);
