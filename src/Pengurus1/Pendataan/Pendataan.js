...
import {styles} from './Stylependataan';
...
class Pendataan extends Component {
  ...
  Adddata = () => {
    ...
    fetch(
      `https://nsj-trash.herokuapp.com/api/pengurus1/pendataan/${this.props.route.params.item.user_id}`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${this.props.userToken.userReducer.token}`,
      ...
      .then((response) => {
        ...
        if (response.status === 'success') {
          ...
          this.props.navigation.navigate('Harusjemput');
...
export default connect(mapStateToProps)(Pendataan);
